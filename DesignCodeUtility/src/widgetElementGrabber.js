  ...
  return Promise.map(widgets, widget => {
    ...
    return endPointTransceiver.getWidget([widget.repositoryId]).then(results => {
      ...
      if (nonInstanceElementPromise) {
        ...
        // Turns out that some elementized widgets have no layouts so look both ways to be sure.
        if (descriptor.editableWidget &&
          (descriptor.layouts.length || (results.data.fragments && results.data.fragments.length))) {
