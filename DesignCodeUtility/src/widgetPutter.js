...
function putWidgetInstanceFile(metadata, path, endpoint, transform) {
  ...
  return buildSource(metadata, path, transform).then(body => {
    if (body) {  
...
/**
 * Builds the source for the file
 * @param metadata
 * @param path
 * @param transform
 * @returns a Bluebird promise
 */
function buildSourceRequest(metadata, path, transform) {

  // Build the basic body.
  const body = request().fromPathAs(path, "source").withEtag(metadata.etag)

  // If transform not needed, return what we have
  if (!transform) {
    return body
  }

  // Work out the correct version of the widget descriptor
 return getMatchingWidgetDescriptorId(metadata, path).then(widgetDescId => {

    // Replace the substitution value in the file with the IDs on the target system.
    return body.replacing(constants.widgetInstanceSubstitutionValue, 
      `#${widgetDescId}-${metadata.instance.repositoryId}`)
  })
}
const buildSource = Promise.method(buildSourceRequest)

/**
 * Get the ID of the matching widget descriptor
 * @param metadata
 * @param path
 * @returns A Bluebird promise
 */
function determineMatchingWidgetDescriptorId(metadata, path) {
    // Check to see if we have the correct version
    if (metadata.instance.version !== metadata.instance.descriptorVersion) {
      // Lookup the correct version
      return readMetadata(path, constants.widgetMetadataJson).then(widgetMetadata => {
        if (widgetMetadata) {
          return widgetMetadata.repositoryId
        }
      })
    }

    // Assume we have the correct version
    return metadata.instance.descriptorRepositoryId
}
const getMatchingWidgetDescriptorId = Promise.method(determineMatchingWidgetDescriptorId)
