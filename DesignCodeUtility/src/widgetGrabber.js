...
function grabRequestedWidgets(widgetName) {
  ...
  return Promise.all([
    ...
    // At the last stage, warn the user about any duplicate user widgets.
    return endPointTransceiver.getAllWidgetInstances()
      .then(results => checkForDuplicateUserWidgets(results.data.items))
  })
}
...
/**
 * Walk through all the user widgets looking for duplicates.
 * @param widgets
 */
const checkForDuplicateUserWidgets = Promise.method(widgets => {

  const userWidgetTypes = new Set()

  // For all user created widgets...
  widgets.filter(widget => widget.source == 101)
    .forEach(widget => {

      // See if we have seen this widget type more than once.
      if (userWidgetTypes.has(widget.widgetType)) {

        // Tell the user something looks iffy.
        warn("multipleUserWidgetVersions",
          {
        ...
        // Not seen this - keep a note of it.
        userWidgetTypes.add(widget.widgetType)
      }
  })
})
