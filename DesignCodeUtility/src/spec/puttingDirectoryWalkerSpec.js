...
const toPlatformPath = require("./platform").toPlatformPath
...
describe("Putting Directory Walker", () => {
  ...
  it("should handle global element files correctly", () => {
    ...
    self.metadata.widgetElementExistsOnTarget.returns(true)
  ...
  it("should handle base widget files correctly", () => {
    ...
    // CCDS-11374: test for child elements that don't exist for an existing widget
    self.metadata.widgetElementExistsOnTarget.returns(false)

    self.classifier.classify.returns(PuttingFileType.ELEMENT_METADATA)
    self.listeners.file("/Users/bmorrow/widget/Duey/element/Brandine", {name: "elementMetadata.json"}, emptyFunction)

    self.classifier.classify.returns(PuttingFileType.ELEMENT_JAVASCRIPT)
    self.listeners.file("/Users/bmorrow/widget/Duey/element/Brandine", {name: "element.js"}, emptyFunction)

    expect(self.pathTypeMap.size).toEqual(12)
    expect(self.classifier.classify).toHaveBeenCalled()
    ...
    // CCDS-11374: test for child elements that don't exist for an existing widget
    expect(self.logger.warn).toHaveBeenCalledWith("cannotUpdateElement", {path: toPlatformPath("/Users/bmorrow/widget/Duey/element/Brandine/elementMetadata.json")})
    expect(self.logger.warn).toHaveBeenCalledWith("cannotUpdateElement", {path: toPlatformPath("/Users/bmorrow/widget/Duey/element/Brandine/element.js")})
  })
