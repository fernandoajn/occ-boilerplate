...
describe("main", () => {
  ...
  beforeEach(() => {
    ...
    mockery.mockModules(self,
      '../logger', '../state', '../endPointTransceiver', '../metadata', '../grabber', '../widgetPutter', '../putter', '../putterUtils',
  ...
  it("should cope with grab returning null", done => {

    self.commander.grab = true
    self.grabber.grab.returns(null)

    self.mainModule.main().then(() => {

      expect(self.endPointTransceiver.init).toHaveBeenCalledWith("http://somehost:8090", "admin", "admin", undefined, undefined, undefined)
      expect(self.grabber.grab).toHaveBeenCalledWith("http://somehost:8090", false, null)
      expect(self.logger.info).not.toHaveBeenCalled()
      done()
    })
  })
