...
function writeTextSnippetsForLocale(localeName, results) {
  ...
  results.data.custom && Object.keys(results.data.resources).forEach(outerKey => {
    ...
    // Temporary fix for CCDS-11389.
    if (typeof (results.data.resources[outerKey]) === 'object') {
      ...
      if (results.data.custom[outerKey])
        results.data.resources[outerKey] = results.data.custom[outerKey];
      else
        results.data.custom[outerKey] = results.data.resources[outerKey];
    }
  })
