...
const widgetElementExistsOnTarget = require("./metadata").widgetElementExistsOnTarget
        ...
        if (!isTrackedFile(fullPath) && !hidden(fileStat)) {
          ...
          switch (fileType) {
            ...
            case PuttingFileType.WIDGET_INSTANCE_LESS:
              ...
              // In the case of a widget element we need to be sure this exists
              // before trying to put the widget.
              case PuttingFileType.ELEMENT_METADATA:
              case PuttingFileType.ELEMENT_JAVASCRIPT:
    
                if (widgetExistsOnTarget(fullPath)) {
                  if (!widgetElementExistsOnTarget(fullPath)) {
                    warn("cannotUpdateElement", {path: fullPath})
                  }
                  else {
                    paths.otherPaths.push(fullPath)
                  }
