...
function getMatchingWidgetInstance(path) {
  ...
  if (matchingWidgetInstance) {
    ...
    const widgetInstanceMetadata = {
      ...
      descriptorVersion: matchingWidgetInstance.descriptor.version,
...
function getMatchingWidget(path) {
  ...
  // If widget instance, version may be different
  const instanceMetadata = readMetadataFromDisk(path, constants.widgetInstanceMetadataJson)
  if (instanceMetadata && instanceMetadata.version) {
    version = instanceMetadata.version
  }

  // See if we can find a widget with the same name and version.
  const matchingWidget = cache.widgetDescriptors.find(
    widget => widget.version === version && widget.displayName === metadata.displayName)
